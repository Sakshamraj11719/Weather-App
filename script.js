...
async function updateAll(lat, lon, label, cityData = null) {
  try {
    ...
    // Call via window.setCurrent so all patches (wind compass, data cache) run
    window.setCurrent ? window.setCurrent(cw) : setCurrent(cw);
    setAQI(aq);
    setForecast(fw);
...
/* ── Realistic Canvas Compass ── */
let _compassCurrentDeg = 0;
let _compassTargetDeg  = 0;
...
function drawCompass(angleDeg) {
  ...
  const clrFace      = isDay ? '#dde4f0' : '#111827';
  const clrRing1     = isDay ? '#c8d2e8' : '#1f2937';
  const clrRing2     = isDay ? '#b8c4de' : '#374151';
  const clrTick      = isDay ? '#6b7280' : '#9ca3af';
  const clrTickMaj   = isDay ? '#374151' : '#d1d5db';
  const clrCardOther = isDay ? '#374151' : '#d1d5db';
  const clrDegLabel  = isDay ? '#4b5563' : '#6b7280';

  ctx.clearRect(0, 0, W, H);

  // Outer bezel
  const bezelGrad = ctx.createRadialGradient(cx, cy - R * 0.2, R * 0.5, cx, cy, R + 4);
  bezelGrad.addColorStop(0, isDay ? '#c8d2e8' : '#1e293b');
  bezelGrad.addColorStop(1, isDay ? '#8899bb' : '#0a0f1a');
  ctx.beginPath();
  ...
  for (let i = 0; i < 72; i++) {
    ...
    const isMaj = i % 9 === 0;
    const r1 = R + 3, r2 = isMaj ? R - 2 : R + 0.5;
    ctx.beginPath();
    ctx.moveTo(cx + Math.cos(a) * r1, cy + Math.sin(a) * r1);
    ctx.lineTo(cx + Math.cos(a) * r2, cy + Math.sin(a) * r2);
    ctx.strokeStyle = isMaj ? (isDay ? '#8899bb' : '#374151') : (isDay ? '#aab6cc' : '#1f2937');
  ...
  // Inner rings
  [R * 0.92, R * 0.82].forEach((r, i) => {
    ctx.beginPath();
    ctx.arc(cx, cy, r, 0, Math.PI * 2);
    ctx.strokeStyle = i === 0 ? clrRing1 : clrRing2;
    ctx.lineWidth = 0.8;
    ctx.stroke();
  });
  ...
  for (let deg = 0; deg < 360; deg += 5) {
    const isMaj10 = deg % 10 === 0;
    const isMaj30 = deg % 30 === 0;
    const a = (deg / 360) * Math.PI * 2 - Math.PI / 2;
    const r1 = R * 0.92;
    const r2 = isMaj30 ? R * 0.74 : isMaj10 ? R * 0.80 : R * 0.87;
    ctx.beginPath();
    ctx.moveTo(cx + Math.cos(a) * r1, cy + Math.sin(a) * r1);
    ctx.lineTo(cx + Math.cos(a) * r2, cy + Math.sin(a) * r2);
    ctx.strokeStyle = isMaj30 ? clrTickMaj : clrTick;
    ctx.lineWidth   = isMaj30 ? 1.5 : isMaj10 ? 0.9 : 0.5;
    ctx.stroke();
  }
  ...
  for (let deg = 0; deg < 360; deg += 30) {
    const a = (deg / 360) * Math.PI * 2 - Math.PI / 2;
    const r = R * 0.69;
    ctx.fillStyle = clrDegLabel;
    ctx.fillText(deg === 0 ? '360' : String(deg), cx + Math.cos(a) * r, cy + Math.sin(a) * r);
  }
  ...
  cardinals.forEach(({ label, deg }) => {
    const a = (deg / 360) * Math.PI * 2 - Math.PI / 2;
    const isMajCard = label.length === 1;
    const r = isMajCard ? R * 0.58 : R * 0.57;
    ctx.font = isMajCard
      ? `700 ${label === 'N' ? 13 : 12}px ui-sans-serif, system-ui, sans-serif`
      : '500 9px ui-sans-serif, system-ui, sans-serif';
    ctx.fillStyle = label === 'N' ? '#22d3ee' : clrCardOther;
    ctx.fillText(label, cx + Math.cos(a) * r, cy + Math.sin(a) * r);
  });

  // Needle
  // NOTE: the tick-mark / cardinal-letter formula above uses `a = deg/360*2π - π/2`
  // to place a POINT via cos(a)/sin(a). The needle is drawn as a path along the
  // local y-axis and rotated with ctx.rotate(), which needs a DIFFERENT angle
  // (no -π/2 term) to land on that same point. Mixing the two formulas was the
  // bug that made the needle point ~90° away from where the dial's own labels
  // (and the "Direction" text) said it should.
  const needleAngle = (angleDeg / 360) * Math.PI * 2;
  const nLen = R * 0.42, sLen = R * 0.32, nWidth = 5;
  ...
  ctx.fillStyle = isDay ? '#94a3b8' : '#374151';
  ctx.fill();
  ...
  // Glare highlight
  const glare = ctx.createRadialGradient(cx - R * 0.3, cy - R * 0.35, 0, cx, cy, R);
  glare.addColorStop(0,   isDay ? 'rgba(255,255,255,0.18)' : 'rgba(255,255,255,0.07)');
...
function animateCompass(targetDeg) {
  if (_compassRafId) cancelAnimationFrame(_compassRafId);

  _compassCurrentDeg = ((_compassCurrentDeg % 360) + 360) % 360;
  targetDeg          = ((targetDeg % 360) + 360) % 360;

  let diff = targetDeg - _compassCurrentDeg;
  if (diff >  180) diff -= 360;
  if (diff < -180) diff += 360;
  _compassTargetDeg = _compassCurrentDeg + diff;

  function step() {
    _compassCurrentDeg += (_compassTargetDeg - _compassCurrentDeg) * 0.08;
    drawCompass(_compassCurrentDeg);
    if (Math.abs(_compassTargetDeg - _compassCurrentDeg) > 0.3) {
      _compassRafId = requestAnimationFrame(step);
    } else {
      _compassCurrentDeg = _compassTargetDeg;
      drawCompass(_compassCurrentDeg);
    }
  }
  step();
...
document.getElementById('btnTheme')?.addEventListener('click', () => {
  setTimeout(() => drawCompass(_compassCurrentDeg), 50);
});
...
function updateWindCompass(cw) {
  ...
  // wind.deg = direction wind is blowing FROM (meteorological convention,
  // also what OpenWeather returns and what the "From X (deg°)" label below
  // displays). Point the needle straight at that direction so the dial
  // visually agrees with the text instead of pointing 180° away from it.
  animateCompass(deg);

  // Cardinal label still describes where wind comes FROM (standard weather convention)
  const cardinalEl = document.getElementById('compassCardinalBig');
  if (cardinalEl) cardinalEl.textContent = degreesToCardinal(deg);

  const dispSpeed = u === 'imperial'
    ? `${(speed * 2.237).toFixed(1)} mph`
    : `${(speed * 3.6).toFixed(1)} km/h`;
  const dispGust = gust != null
    ? (u === 'imperial'
        ? `${(gust * 2.237).toFixed(1)} mph`
  ...
  // Show both: where wind comes FROM and its degree
  if (dirEl) dirEl.textContent = `From ${degreesToCardinal(deg)} (${deg}°)`;
...
window.addEventListener('load', () => {
  ...
  // NOTE: window.setCurrent is already patched above (at line ~2347) to include
  // updateWindCompass. We only need to add lastCurrentWeather caching here —
  // the _sc reference already has updateWindCompass baked in.
