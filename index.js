...
async function initDatabase() {
  ...
  const stmts = {
    ...
    // Password reset
    upsertPasswordReset:  db.prepare('INSERT OR REPLACE INTO password_resets (email, otp, reset_token, expires_at) VALUES (?, ?, ?, ?)'),
    getPasswordReset:     db.prepare('SELECT * FROM password_resets WHERE email = ?'),
    deletePasswordReset:  db.prepare('DELETE FROM password_resets WHERE email = ?'),
  ...
  // Step 1 — send reset OTP: POST /api/auth/forgot-password
  app.post('/api/auth/forgot-password', async (req, res) => {
    try {
      ...
      const existing = stmts.getPasswordReset.get(email);
      ...
      const expiresAt  = Date.now() + 10 * 60 * 1000;
      stmts.upsertPasswordReset.run(email, otp, resetToken, expiresAt);
  ...
  // Step 2 — verify reset OTP: POST /api/auth/verify-reset-otp
  app.post('/api/auth/verify-reset-otp', (req, res) => {
    try {
      ...
      const row = stmts.getPasswordReset.get(email);
      if (!row)
        return res.status(400).json({ success: false, message: 'No reset request found. Please start over.' });
      if (Number(row.expires_at) < Date.now())
        return res.status(400).json({ success: false, message: 'Code expired. Please request a new one.' });
      if (String(row.otp) !== String(otp))
        return res.status(400).json({ success: false, message: 'Incorrect code. Please try again.' });

      return res.json({ success: true, resetToken: row.reset_token });
    } catch (err) {
  ...
  // Step 3 — set new password: POST /api/auth/reset-password
  app.post('/api/auth/reset-password', async (req, res) => {
    try {
      ...
      const row = stmts.getPasswordReset.get(email);
      if (!row || row.reset_token !== resetToken)
      ...
      stmts.deletePasswordReset.run(email);
      stmts.deleteUserSessions.run(email); // Invalidate all sessions for security
